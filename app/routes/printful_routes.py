...
from flask import Blueprint, current_app, flash, render_template, request, session, jsonify
...
def _is_admin() -> bool:
    ...
    admin_key = current_app.config.get("MARKETING_ADMIN_KEY") or current_app.config.get("PRINTFUL_ADMIN_KEY")
    if admin_key:
...
def _get_ttl() -> int:
    raw = os.getenv("PRINTFUL_CACHE_TTL") or str(current_app.config.get("PRINTFUL_CACHE_TTL", "300"))
