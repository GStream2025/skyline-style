...
{% block extra_head %}
  {% set _env = (ENV|default('')|string|lower|trim) %}
  ...
  {% macro safe_url(ep, fb='') -%}
    {%- if ep and _vf and ep in _vf -%}{{ url_for(ep) }}
    {%- else -%}{{ fb }}
    {%- endif -%}
  {%- endmacro %}

  {# CSRF robusto: función o string, o vacío #}
  {% set _csrf = '' %}
  ...
  <style{% if _nonce %} nonce="{{ _nonce|e }}"{% endif %}>
    ...
    .ss-ve__card{max-width:580px;width:100%;border-radius:26px;border:1px solid rgba(148,163,184,.22);background:rgba(255,255,255,.92);box-shadow:0 22px 60px rgba(2,6,23,.12);overflow:hidden}
    .ss-ve__head{padding:22px 20px;border-bottom:1px solid rgba(148,163,184,.16)}
    ...
    .ss-ve__btn{display:inline-flex;align-items:center;justify-content:center;gap:10px;padding:12px 14px;border-radius:999px;border:1px solid rgba(15,23,42,.14);background:rgba(255,255,255,.9);font-weight:900;cursor:pointer;text-decoration:none;color:rgba(11,18,32,.92);user-select:none;min-height:44px}
    ...
    @media (prefers-reduced-motion:reduce){
      .ss-ve__btn{transform:none!important}
  ...
  <section class="ss-ve" aria-label="Verificación de correo">
    <div class="ss-ve__card" role="region" aria-labelledby="veTitle">
      <div class="ss-ve__head">
        ...
        <p class="ss-ve__sub">
          {% if email %}
            Enviamos un enlace a <b>{{ email|e }}</b>.
          {% else %}
            Enviamos un enlace a tu correo.
          {% endif %}
      ...
      <div class="ss-ve__body">
        ...
        <div class="ss-ve__note" role="note">
          ...
          Si pedís reenvío, esperá <b>30s</b>.
        </div>
        ...
        <form id="resendForm" method="post" action="{{ resend_ep }}">
          ...
          <noscript>
            <div class="ss-ve__note" style="margin-top:12px">
              ...
              <button class="ss-ve__btn ss-ve__btn--primary" type="submit"
...
(() => {
  ...
  const send = async () => {
    ...
    try {
      ...
      // libera el cooldown si falló duro (mejor UX)
      setCd(0);
    } finally {
      stopLoading();
      updateBtn();
  ...
  if (btn) btn.addEventListener('click', send, { passive: true });
